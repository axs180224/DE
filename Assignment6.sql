...
--Exercise 1: Get all the number of users created per year
SELECT YEAR(created_ts) AS created_year, COUNT(*) AS user_count
FROM users
GROUP BY YEAR(created_ts) -- always included non aggregated columns in group by
ORDER BY created_year;

--Exercise 2: Get the day name of the birth days for all the users born in the month of May.
SELECT user_id, user_dob,user_email_id, DATENAME(dw, user_dob) AS user_day_of_birth --dw is dayweek
--DATENAME returns specific part of date
FROM users
WHERE MONTH(user_dob) = 5;

--Exercise 3: Get the names and email ids of users added in year 2019.
SELECT user_id, CONCAT_WS(' ', user_first_name, user_last_name) AS user_name, 
user_email_id, created_ts, YEAR(created_ts) created_year
FROM users
...
--Exercise 8
SELECT
   CASE WHEN DATEPART(WEEKDAY, order_date) IN (1, 7) 
   THEN 'Weekend days'
...
FROM
    orders
	WHERE
    MONTH(order_date) = 1 AND YEAR(order_date) = 2014
